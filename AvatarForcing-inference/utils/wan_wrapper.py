...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(os.path.join(os.path.dirname(__file__), "../wan_models/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth"),
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name=os.path.join(os.path.dirname(__file__), "../wan_models/Wan2.1-T2V-1.3B/google/umt5-xxl/"), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path=os.path.join(os.path.dirname(__file__), "../wan_models/Wan2.1-T2V-1.3B/Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
